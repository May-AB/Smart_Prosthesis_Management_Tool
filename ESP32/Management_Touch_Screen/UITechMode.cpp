...
// ---------- UITechMode-owned state (not global) ----------
...
static void sliderEventCbAnim(lv_event_t *e) {
...
static void sliderEventCbUpdatedVal(lv_event_t *e) {
  // Event callback placeholder
}

static void sliderEventCbUpdatedValMotor(lv_event_t *e) {
  // Event callback placeholder
}
...
ReturnUnsavedParam checkUnsaveSensorParam() {
  ...
  if (currentEditSensorSlidersVec.size() > 0 && currentEditSensorId != -1) {
    for (const auto &[paramName, param] :
         sensors[currentEditSensorId].function.parameters) {
      if (param.currentVal !=
          lv_slider_get_value(currentEditSensorSlidersVec[i])) {
        retParamsId.push_back(i);
        retNewVals.push_back(
            lv_slider_get_value(currentEditSensorSlidersVec[i]));
      }
      i++;
    }
...
static void motorsActivity(lv_obj_t *parent) {
  ...
  indic = lv_meter_add_scale_lines(meter, scale, HEX_DARK_BLUE, HEX_DARK_BLUE,
                                   false, 0);
  lv_meter_set_indicator_start_value(meter, indic, 0);
  lv_meter_set_indicator_end_value(meter, indic, 20);
...
static void discardSensorBtnClickEvent(lv_event_t *e) {
  ...
  if (unsaveStruct.paramsId.size() > 0 && currentEditSensorId != -1) {
    ...
    for (const auto &[paramName, param] :
      ...
      if (param.currentVal !=
          lv_slider_get_value(currentEditSensorSlidersVec[i])) {
        lv_slider_set_value(currentEditSensorSlidersVec[i], param.currentVal,
                            LV_ANIM_ON);
        lv_event_send(currentEditSensorSlidersVec[i], LV_EVENT_VALUE_CHANGED,
                      NULL);
      }
      i++;
    }
...
static void saveNewMotorValToStruct(ReturnUnsavedParam motorThsStruct) {
  if (motorThsStruct.itemId != -1) {
    motors[motorThsStruct.itemId].safetyThreshold.currentVal =
        motorThsStruct.newVals[0];
  }
}

static void saveNewSensorsValToStruct(ReturnUnsavedParam sensorsStruct) {
  if (sensorsStruct.itemId == -1)
    return;
  size_t i = 0;
  int j = 0;
  for (auto &[name, param] :
       sensors[sensorsStruct.itemId].function.parameters) {
...
static void saveBtnTechMotorApproved(lv_event_t *e) {
  ...
  lv_obj_t *currMsgBox = lv_msgbox_create(
      NULL, LV_SYMBOL_OK, "Changes have been saved.\nProthesis is updated.",
      NULL, true);
  lv_obj_align(currMsgBox, LV_ALIGN_CENTER, 0, 0);
}

static void saveBtnTechSensorApproved(lv_event_t *e) {
  if (sendNewSwitchesMsgBox) {
    lv_msgbox_close(sendNewSwitchesMsgBox);
    sendNewSwitchesMsgBox = NULL;
  }
...
static void saveBtnTechMotorClickEvent(lv_event_t *e) {
  ReturnUnsavedParam unsaveMotorThs = checkUnsaveMotorParam();

  if (unsaveMotorThs.paramsId.size() == 0) {
    lv_obj_t *currMsgBox =
        lv_msgbox_create(NULL, "Nothing to save...", NULL, NULL, true);
    lv_obj_align(currMsgBox, LV_ALIGN_CENTER, 0, 0);
  } else {
    if (isDemoYaml.test_and_set()) {
      saveNewMotorValToStruct(unsaveMotorThs);
      lv_obj_t *currMsgBox = lv_msgbox_create(
          NULL, LV_SYMBOL_OK, "Changes have been saved.", NULL, true);
      lv_obj_align(currMsgBox, LV_ALIGN_CENTER, 0, 0);
    } else {
      isDemoYaml.clear();
      ...
      sendNewSwitchesMsgBox = lv_msgbox_create(
          NULL, "Saving changes...",
          "Sending new motor's safety threshold to prosthesis.", NULL, false);
...
static void saveBtnTechSensorClickEvent(lv_event_t *e) {
  ReturnUnsavedParam unsaveSensors = checkUnsaveSensorParam();
  ...
  if (unsaveSensors.paramsId.size() == 0) {
    ...
    if (isDemoYaml.test_and_set()) {
      ...
      sendNewSwitchesMsgBox = lv_msgbox_create(
          NULL, "Saving changes...",
          "Sending new sensor's parameters to prosthesis.", NULL, false);
      lv_obj_center(sendNewSwitchesMsgBox);

      lv_refr_now(NULL);

      SendSensorParamChangeReq(unsaveSensors.itemId, unsaveSensors.paramsId,
...
static void eventViewEditMotors(lv_event_t *e) {
  ...
  if (((currentEditMotorId != -1) && (currentEditMotorId != id)) ||
      motorTriggeredByTechTab) {
    if (checkUnsaveMotorParam().paramsId.size() != 0) {
      ...
      lv_obj_t *currMsgBox =
          lv_msgbox_create(NULL, "Changes were not saved",
                           "Please choose Save/Discard", NULL, true);
      lv_obj_align(currMsgBox, LV_ALIGN_CENTER, 0, 0);
      ...
      lv_obj_add_style(slider, &style_disable,
      ...
      lv_obj_add_event_cb(slider, sliderEventCbAnim, LV_EVENT_VALUE_CHANGED,
                          valueLabel);
      lv_obj_add_event_cb(slider, sliderEventCbUpdatedValMotor,
      ...
      saveBtnTechMotors =
          createNewBtn(parent, 72, 32, LV_ALIGN_TOP_MID, -50, yOffset + 8,
                       "Save", HEX_GREEN, HEX_WHITE, &lv_font_montserrat_14);
      objsToDeleteMotors.push_back(saveBtnTechMotors);
      lv_obj_t *discardBtn =
          createNewBtn(parent, 72, 32, LV_ALIGN_TOP_MID, 30, yOffset + 8,
                       "Discard", HEX_RED, HEX_WHITE, &lv_font_montserrat_14);
      ...
      lv_obj_add_event_cb(saveBtnTechMotors, saveBtnTechMotorClickEvent,
                          LV_EVENT_CLICKED, NULL);
      lv_obj_add_event_cb(saveBtnTechMotors, saveBtnTechMotorApproved,
                          EVENT_SENSOR_CHANGED_SECC, NULL);
      lv_obj_add_event_cb(discardBtn, discardMotorBtnClickEvent,
                          LV_EVENT_CLICKED, NULL);
    }
    ...
    lv_obj_add_event_cb(slider, sliderEventCbAnim, LV_EVENT_VALUE_CHANGED,
                        valueLabel);
    lv_obj_add_event_cb(slider, sliderEventCbUpdatedValMotor, LV_EVENT_RELEASED,
                        NULL);
    yOffset += 20;
    lv_obj_set_user_data(slider, valueLabel);
    ...
    saveBtnTechMotors =
        createNewBtn(parent, 72, 32, LV_ALIGN_TOP_MID, -50, yOffset + 8, "Save",
                     HEX_GREEN, HEX_WHITE, &lv_font_montserrat_14);
    objsToDeleteMotors.push_back(saveBtnTechMotors);
    lv_obj_t *discardBtn =
        createNewBtn(parent, 72, 32, LV_ALIGN_TOP_MID, 30, yOffset + 8,
                     "Discard", HEX_RED, HEX_WHITE, &lv_font_montserrat_14);
    ...
    lv_obj_add_event_cb(saveBtnTechMotors, saveBtnTechMotorApproved,
                        EVENT_SENSOR_CHANGED_SECC, NULL);
    lv_obj_add_event_cb(discardBtn, discardMotorBtnClickEvent, LV_EVENT_CLICKED,
                        NULL);
  }
}

static void eventViewAndEditSensors(lv_event_t *e) {
  lv_obj_t *parent = (lv_obj_t *)lv_event_get_user_data(e);
  lv_obj_t *obj = lv_event_get_target(e);
  int id = -1;
  if (obj) {
  ...
  if (((currentEditSensorId != -1) && (currentEditSensorId != id)) ||
      sensorTriggeredByTechTab) {
    if (checkUnsaveSensorParam().paramsId.size() != 0) {
      ...
      if (sensorTriggeredByTechTab) {
        ...
        for (const auto &param : sensor.function.parameters) {
          ...
          lv_obj_add_event_cb(slider, sliderEventCbAnim, LV_EVENT_VALUE_CHANGED,
                              valueLabel);
          lv_obj_add_event_cb(slider, sliderEventCbUpdatedVal,
                              LV_EVENT_RELEASED, NULL);
        ...
        lv_obj_set_style_text_font(labelSensorStatus, &lv_font_montserrat_12,
                                   0);
        ...
        lv_label_set_text_fmt(labelSensorStatus, "Status: %s",
                              sensor.status.c_str());
        lv_label_set_text_fmt(labelSensorType, "Type: %s", sensor.type.c_str());

        lv_obj_add_event_cb(saveBtnTechSensors, saveBtnTechSensorClickEvent,
                            LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(saveBtnTechSensors, saveBtnTechSensorApproved,
                            EVENT_SENSOR_CHANGED_SECC, NULL);
        lv_obj_add_event_cb(discardBtn, discardSensorBtnClickEvent,
                            LV_EVENT_CLICKED, NULL);
      }
    }
    ...
    for (const auto &param : sensor.function.parameters) {
      ...
      lv_obj_add_event_cb(slider, sliderEventCbAnim, LV_EVENT_VALUE_CHANGED,
                          valueLabel);
      lv_obj_add_event_cb(slider, sliderEventCbUpdatedVal, LV_EVENT_RELEASED,
                          NULL);
...
static void techTabviewEventCb(lv_event_t *e) {
...
static lv_obj_t *createMotorsTabTech(lv_obj_t *parent) {
...
static lv_obj_t *createSensorsTabTech(lv_obj_t *parent) {
  lv_obj_t *dropdown = lv_dropdown_create(parent);
  lv_obj_align(dropdown, LV_ALIGN_TOP_LEFT, -10, -10);
  char options[256];
...
void createControlsForTech(lv_obj_t *parent) {
  ...
  lv_obj_set_style_bg_color(techTabBtns, lv_palette_darken(LV_PALETTE_GREY, 3),
                            0);
  lv_obj_set_style_text_color(techTabBtns,
                              lv_palette_lighten(LV_PALETTE_GREY, 5), 0);
  lv_obj_set_style_border_side(techTabBtns, LV_BORDER_SIDE_LEFT,
                               LV_PART_ITEMS | LV_STATE_CHECKED);
  lv_obj_set_style_bg_opa(techTabBtns, LV_OPA_COVER, 0);

  techTabMotors =
      lv_tabview_add_tab(techTabview, "Motors\n" LV_SYMBOL_SETTINGS);
  techTabSensors =
      lv_tabview_add_tab(techTabview, "Sensors\n" LV_SYMBOL_SETTINGS);
  techTabMotorsActivity = lv_tabview_add_tab(techTabview, "Motors\nActivity");
  lv_obj_clear_flag(lv_tabview_get_content(techTabview),
                    LV_OBJ_FLAG_SCROLLABLE);
  lv_obj_add_event_cb(techTabview, techTabviewEventCb, LV_EVENT_VALUE_CHANGED,
                      NULL);
  ...
  lv_obj_add_event_cb(dropdownSensors, eventViewAndEditSensors,
                      LV_EVENT_VALUE_CHANGED, techTabSensors);
  lv_obj_add_event_cb(dropdownMotors, eventViewEditMotors,
                      LV_EVENT_VALUE_CHANGED, techTabMotors);
}

// Resets all edit-session state — called by UINavigation on screen transitions
void clearEditState() {
  for (auto &obj : objsToDeleteSensors) {
    lv_obj_del(obj);
  }
  for (auto &obj : objsToDeleteMotors) {
    lv_obj_del(obj);
  }
  objsToDeleteSensors.clear();
  objsToDeleteMotors.clear();
