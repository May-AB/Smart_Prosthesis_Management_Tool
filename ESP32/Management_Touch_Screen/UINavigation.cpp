...
// Non-blocking LVGL timer used for YAML poll loop — avoids recursive
// loadYamlStep calls
...
void readYamlFromProtScreenFunction() {
  ...
  lv_obj_add_event_cb(readYamlFromProtScreen, loadYamlStep, LV_EVENT_CLICKED,
                      NULL);
}
...
void setupWelcomeScreen() {
  ...
  lv_obj_add_event_cb(welcomeScreen, changeToMainScreen, LV_EVENT_LONG_PRESSED,
                      NULL);
}
...
void searchClientBLEScreen() {
  ...
  lv_label_set_recolor(label1, true);
  lv_label_set_text(
      label1, "#00007f No Bluetooth connection found for the prosthesis#");
  lv_obj_set_style_text_align(label1, LV_TEXT_ALIGN_CENTER, 0);
  lv_obj_align(label1, LV_ALIGN_CENTER, 0, -50);
  lv_obj_set_style_text_font(label1, &lv_font_montserrat_20, 0);

  lv_obj_t *searchBleBtn =
      createNewBtn(searchBleScreen, 170, 30, LV_ALIGN_CENTER, 0, 10,
                   "Search Again", HEX_ROYAL_BLUE, HEX_BLACK);
  lv_obj_t *testExampleBtn =
      createNewBtn(searchBleScreen, 170, 30, LV_ALIGN_CENTER, 0, 50,
                   "Start DEMO YAML", HEX_YELLOW, HEX_BLACK);

  lv_obj_add_event_cb(searchBleBtn, setSearching, LV_EVENT_PRESSED, NULL);
  lv_obj_add_event_cb(searchBleBtn, searchBleAgainEvent, LV_EVENT_CLICKED,
                      NULL);
...
void passClickEvent(lv_event_t *e) {
  ...
  const char *btn_text = lv_btnmatrix_get_btn_text(
      btn_matrix, lv_btnmatrix_get_selected_btn(btn_matrix));
...
void setupInitialUserScreen() {
  ...
  static char *btnm_map[] = {"User", "\n", "Tech", "Debug",
                             ""}; // The last element must be an empty string
  int num_pointer = sizeof(btnm_map) / sizeof(btnm_map[0]);

  static char **map_ptr = btnm_map;

  lv_obj_t *btnm = createNewMatrixBtnChooseOne(
      initialUserScreen, &map_ptr, num_pointer, LV_ALIGN_CENTER, 0, 20,
      HEX_SKY_BLUE, HEX_DARK_BLUE, HEX_WHITE);
  ...
  lv_obj_set_style_bg_color(passwordScreen, HEX_LIGHT_GRAY,
                            0); // Optional: Set background color
...
static void yamlPollTimerCb(lv_timer_t *timer) {
  ...
  if (yamlStructsReady) {
    ...
    lv_timer_del(timer);
    s_yamlPollTimer = nullptr;
    xTaskCreatePinnedToCore(BLENotifyTask, "BLE Notify Task", 2048, NULL, 2,
                            &bleNotifyTaskHandle, 0);
...
void loadYamlStep(lv_event_t *e) {
  if (hasClient.test_and_set()) {
    ...
    // YAML still arriving — start non-blocking timer instead of
    // blocking/recursing
