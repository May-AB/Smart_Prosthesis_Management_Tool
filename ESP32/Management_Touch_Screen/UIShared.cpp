...
lv_obj_t* createNewMatrixBtnChooseOne(
    ...
    bool big,
    lv_coord_t screenWidth,
    lv_coord_t screenHeight,
    int maxInRow,
    bool needToFreeOldMap)
{
    ...
    *labelsColorPtr = labelsColor;
    if (big) {
        lv_obj_add_event_cb(btnm, designLabelBig, LV_EVENT_DRAW_PART_BEGIN, labelsColorPtr);
    } else {
        lv_obj_add_event_cb(btnm, designLabelSmall, LV_EVENT_DRAW_PART_BEGIN, labelsColorPtr);
    }
...
void getOptionsString(char* destBuffer, size_t destSize, bool isMotors) {
  if (destBuffer == nullptr || destSize == 0) return;
  destBuffer[0] = '\0';
  size_t ind = 0;
  if (isMotors) {
