...
void SendNotifyToClient(const char *msgStr, int msgTypeEnum,
  ...
  if (totalMsgNum > 1) {
    Serial.println("The message is too long, dividing into multiple sends");
  }
...
void SendEmergencyReq(const char *msgStr, int msgTypeEnum,
  ...
  Serial.print("Sending msg:");
  printMsg(&msgBytes);
  pCharacteristic->setValue((uint8_t *)&msgBytes,
                            sizeof(struct msgInterpeterStruct));
...
bool isMsgCorrupted(struct msgInterpeterStruct *structVal) {
  if ((structVal->msgLength != strlen(structVal->msg)) ||
      calculateChecksum(structVal->msg, structVal->msgLength) !=
          (structVal->checksum)) {
    Serial.printf("Got msg %d out of %d.\n", structVal->curMsgCount,
                  structVal->totMsgCount);
    Serial.printf("Received MSG length and desired length are %s, (received "
                  "length: %d, desired length: %d)\n",
                  structVal->msgLength == strlen(structVal->msg) ? "equal"
                                                                 : "not equal",
                  strlen(structVal->msg), structVal->msgLength);
    Serial.printf(
        "Calculate MSG checksum is %s to desired cheksum!\n",
        (structVal->checksum) ==
                (calculateChecksum(structVal->msg, structVal->msgLength))
            ? "equal"
            : "not equal");
...
// Receives the split YAML fields from the Management Touch Screen and copy into
// a buffer on the heap, on first msg it allocates the memory for the buffer.
// Using pointer to pointer cause calloc changes the address of the buffer
void ReciveYAMLField(uint8_t **bufferToUse,
  ...
  if ((structVal.curMsgCount) == 1) {
    *bufferToUse = (uint8_t *)calloc((structVal.totMsgCount) * MAX_MSG_LEN,
                                     sizeof(uint8_t));
    if (!(*bufferToUse)) {
      Serial.println("calloc failed");
    }
  }
  memcpy((*bufferToUse) + ((structVal.curMsgCount - 1) * (MAX_MSG_LEN - 1)),
         structVal.msg, structVal.msgLength);
}
