...
void setup() {
  Serial.begin(115200);
  delay(1500); // milisec
  xTaskCreatePinnedToCore(StartBLEServer, "BLEServer", STACK_SIZE, nullptr, 2,
                          nullptr, 0);
...
#else
  dispDrawBuf =
      (lv_color_t *)malloc(sizeof(lv_color_t) * screenWidth * screenHeight / 2);
#endif
  if (!dispDrawBuf) {
    Serial.println("LVGL dispDrawBuf allocate failed!");
    return;
  }
  lv_disp_draw_buf_init(&drawBuf, dispDrawBuf, NULL,
