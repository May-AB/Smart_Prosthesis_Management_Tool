...
static void gesturesClickEvent(lv_event_t *e) {
...
void createControlsForMain(lv_obj_t *parent) {
  lv_obj_t *returnBtn =
      createNewBtn(parent, 80, 40, LV_ALIGN_BOTTOM_MID, -110, 0, "Return",
                   HEX_RED, HEX_WHITE, &lv_font_montserrat_16);
...
#define MAX_GESTURES_SUPPORTED 16
  ...
  for (int i = 0; i < numFunctionTotal; i++) {
    ...
    if (functions[i].protocolType == FUNC_TYPE_GESTURE) {
      ...
      gesturesMatrix[j] = createNewBtn(
          parent, 90, 30, LV_ALIGN_TOP_RIGHT, -7 - (j % maxInRow) * 95,
          20 + (j / maxInRow) * 35, temp_str, HEX_DARK_BLUE, HEX_WHITE);
      j++;
    }
  }
  ...
  for (int k = 0; k < j; k++) {
    lv_obj_add_event_cb(gesturesMatrix[k], gesturesClickEvent, LV_EVENT_PRESSED,
                        infoLabel);
    lv_obj_add_event_cb(
        gesturesMatrix[k], [](lv_event_t *e) { delay(800); }, LV_EVENT_CLICKED,
        NULL);
    lv_obj_add_event_cb(gesturesMatrix[k], gesturesClickEvent, LV_EVENT_CLICKED,
                        infoLabel);
  }
...
std::vector<int> findNewOffSensor() {
  ...
  for (int i = 0; i < sensors.size(); i++) {
    if ((sensors[i].status.equalsIgnoreCase("ON")) &&
        (!lv_obj_has_state(sensorSwitchVec[i], LV_STATE_CHECKED))) {
      retVec.push_back(i);
    }
  }
  return retVec;
...
std::vector<int> findNewOnSensor() {
  std::vector<int> retVec;
  for (int i = 0; i < sensors.size(); i++) {
    if ((sensors[i].status.equalsIgnoreCase("OFF")) &&
        (lv_obj_has_state(sensorSwitchVec[i], LV_STATE_CHECKED))) {
      retVec.push_back(i);
    }
  }
...
static void discardBtnClickEvent(lv_event_t *e) {
...
static void saveNewSwitchBtnmToStruct() {
  for (size_t i = 0; i < sensors.size(); i++) {
...
static void saveBtnApproved(lv_event_t *e) {
...
static void saveBtnClickEvent(lv_event_t *e) {
  ...
  if ((newOnSensors.size() == 0) && (newOffSensors.size() == 0)) {
    ...
    if (isDemoYaml.test_and_set()) {
      ...
      sendNewSwitchesMsgBox = lv_msgbox_create(
          NULL, "Saving changes...",
          "Sending new sensors states to prosthesis.", NULL, false);
...
void createControlsForSetup(lv_obj_t *parent) {
  ...
  for (const auto &sensor : sensors) {
    ...
    lv_obj_add_style(sensorSwitch, &style_indic_on,
                     LV_PART_INDICATOR | LV_STATE_CHECKED);
    lv_obj_add_style(sensorSwitch, &style_indic_off,
  ...
  saveBtn = createNewBtn(parent, 90, 40, LV_ALIGN_TOP_MID, -50, yOffset + 8,
                         "Save", HEX_GREEN, HEX_WHITE, &lv_font_montserrat_16);
  lv_obj_t *discardBtn =
      createNewBtn(parent, 90, 40, LV_ALIGN_TOP_MID, 50, yOffset + 8, "Discard",
                   HEX_RED, HEX_WHITE, &lv_font_montserrat_16);

  lv_obj_add_event_cb(saveBtn, saveBtnClickEvent, LV_EVENT_CLICKED, NULL);
  lv_obj_add_event_cb(saveBtn, saveBtnApproved, EVENT_SENSOR_CHANGED_SECC,
                      NULL);
